...
#### 1. 回测列表滚动优化
- **问题**：回测交易记录过多时，页面被无限拉长，难以查看底部内容。
- **优化**：为交易列表添加 `max-h-[400px]` 和 `overflow-auto`，并实现表头 `sticky` 固定。
- **效果**：保持页面紧凑，表头始终可见，提升长列表阅读体验。

#### 2. 图表渲染与自适应修复
- **问题**：
  - 切换 TAB 或刷新页面时，图表可能显示为空白。
  - "Price Overview" 小图未能显示完整历史数据，且未填满横坐标。
- **优化**：
  - 在所有图表组件（`PriceChart`, `PriceOverview`, `AttentionChart`）中引入 `ResizeObserver`，监听容器大小变化并自动调整图表尺寸。
  - `PriceOverview` 组件独立请求 4H 周期全量数据，并使用 `timeScale().fitContent()` 确保填满显示区域。
- **效果**：图表在任何布局变化下均能正确渲染，概览图始终展示完整趋势。

#### 3. 图表控件交互修复
- **问题**：成交量占比调整和事件标注开关点击无反应。
- **原因**：`PriceChart` 内部状态未正确提升，导致父组件重新渲染时状态丢失或未触发更新。
- **优化**：将 `volumeRatio` 和 `showEventMarkers` 状态提升至 `DashboardTab` 父组件管理，实现受控组件模式。
- **效果**：控件响应灵敏，状态在组件重绘后保持一致。
